...
# Find the project root by walking upward to the nearest .env file.
...
if [[ -f "$PROJECT_ROOT/.env" ]]; then
  export $(grep -v '^#' "$PROJECT_ROOT/.env" | tr -d '\r' | grep -v '^\s*$' | xargs)
fi
...
CURL_ARGS=(-sk -X "$METHOD" -H "Content-Type: application/json" -H "Authorization: AccessToken=${ACCESS_TOKEN}")
